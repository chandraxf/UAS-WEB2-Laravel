            ...
            <div class="card">
                <div class="card-header">Edit Nilai</div>
                    ...
                    @csrf
                        ...
                        @foreach ($mhs as $u)
                            <option value="{{ $u->user_id }}" {{ $nilai->mahasiswa_id == $u->user_id ? 'selected' : '' }} > {{ $u->user->name }}</option>
                        @endforeach
                        </select>
                            </div>
                        </div>
                     ...
                     <div class="form-row">
                        ...
                        @foreach ($mkl as $k)
                            <option value="{{ $k->id }}" {{ $nilai->makul_id == $k->id ? 'selected' : '' }} >{{ $k->nama_makul}}</option>
                        @endforeach
                        </select>
                            </div>
                        </div>
                     ...
                     <div class="form-row">
                         <label class="col-md-4 text-md-center pl-4" for="nilai">Nilai</label>
                            <div class="col-md-4">
                                <input type="number" name="nilai" class="form-control" placeholder="Masukan nilai ..." value="{{$nilai->nilai}}">
                            </div>
                        </div>
                        </div>
